...
df['Absolute_Asymmetry'] = df['SSA']
#df['Absolute_Asymmetry'] = df['Kp_N']-df['Kp_S']
...
time = df['Time'].values
signal = df['Absolute_Asymmetry'].values
...
for freq, period in zip(peak_frequencies, peak_periods):
    ...
    #plt.text(freq, np.max(lombscargle)-y_step_size/3, f'{period/12.0:.2f} yr', color='red',horizontalalignment='right', verticalalignment='center', fontsize=12, rotation=90)
